...
async function sendEmailNotification(oauth2Client, toEmail, eventDetails, clientName) {
  ...
  const subject = 'Návrh Schůzky od vašeho Special Agent 23';
...
async function findFreeSlot(oauth2Client, preferredStart, preferredEnd) {
  ...
  if (!existing.data.items || existing.data.items.length === 0) {
    return { start: preferredStart, end: preferredEnd }; // No conflict
  }
  
  console.log(`[CALENDAR] Conflict at ${preferredStart}, searching for free slot...`);
  
  // Find next free slot in next 7 days
  ...
  // No free slot found, return original (will create with conflict)
...
export async function handleAction(supabase, userId, cpId, classification, emailData) {
  ...
  if (actionType === 'event') {
    ...
    try {
      ...
      if (oauth2Client) {
        // Find free slot (avoids conflicts)
  ...
  if (actionType.includes('todo')) {
    ...
    try {
      ...
      let dueDate = new Date(todoDetails.due_date); const today = new Date(); today.setHours(0,0,0,0); if (isNaN(dueDate) || dueDate < today) { dueDate = today; } todoDetails.due_date = dueDate.toISOString().split('T')[0]; await supabase.from('todos').insert({
        user_id: userId,
        cp_id: cpId,
      ...
      console.log(`[ACTION] Created todo: ${todoDetails.description}`);
